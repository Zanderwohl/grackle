...
/// The catalogue until there are data files, and no longer.
///
/// Every weapon the game had before this refactor, in the shape it now has,
/// plus the secondaries that make the new action variants reachable. Written
/// in Rust so that the layers above can be built and tested before anything
/// reads a file — and deleted the moment they can.
#[doc(hidden)]
pub fn debug_catalogue() -> WeaponCatalogue {
    fn weapon(name: &str, primary: Mounted, secondary: Mounted, magazine: Magazine) -> Weapon {
        Weapon {
            id: WeaponId::of(name),
    ...
    let weapons = vec![
        weapon(
            ...
            Mounted {
                action: WeaponAction::Scope(ScopeSpec { zoom: 2.5 }),
                cadence: Cadence::Semi,
                cost: Cost::Free,
            },
            Magazine { clip: Some(12), reserve: 60, reload: 1.0, reload_one_at_a_time: false },
        ),
        weapon(
            "rocket_launcher",
            semi(WeaponAction::Projectile(ProjectileSpec::ROCKET), 1),
            semi(WeaponAction::Projectile(ProjectileSpec::RPG), 1),
            Magazine { clip: Some(4), reserve: 20, reload: 0.8, reload_one_at_a_time: true },
        ),
        weapon(
            "pipe_launcher",
            semi(WeaponAction::Projectile(ProjectileSpec::PIPE_BOMB), 1),
            Mounted::EMPTY,
            Magazine { clip: Some(6), reserve: 24, reload: 1.2, reload_one_at_a_time: false },
        ),
        weapon(
            "flamethrower",
            auto(WeaponAction::Flame(FlameSpec::FLAMETHROWER), 0.1, 1),
            auto(
                WeaponAction::Extinguish(ExtinguishSpec { range: 6.0, spread: 0.28 }),
                0.75,
                20,
            ),
            // No clip: the whole supply is loaded, and it never reloads.
            Magazine { clip: None, reserve: 200, reload: 0.0, reload_one_at_a_time: false },
        ),
        weapon(
            "medi_wrench",
            Mounted {
                action: WeaponAction::HealBeam(HealBeamSpec { range: 8.0, heal_per_second: 24 }),
                cadence: Cadence::Automatic { interval: 0.1 },
                cost: Cost::Free,
            },
            Mounted {
                action: WeaponAction::Invulnerable(InvulnSpec { duration: 8.0, cooldown: 0.0 }),
                cadence: Cadence::Semi,
                cost: Cost::Charge { fraction: 1.0 },
            },
            Magazine::BOTTOMLESS,
        ),
    ];
    ...
    for weapon in weapons {
        catalogue.weapons.insert(weapon.id, weapon);
    }

    let mut mercenary = Loadout::default();
    let mut carry = |slot: Slot, permitted: &[&str]| {
        let permitted: Vec<WeaponId> = permitted.iter().copied().map(WeaponId::of).collect();
        let default = permitted[0];
        mercenary.slots.insert(slot, SlotChoices { permitted, default });
    };
    carry(Slot::Primary, &["laser_rifle", "flamethrower"]);
    carry(Slot::Secondary, &["rocket_launcher", "pipe_launcher"]);
    carry(Slot::Melee, &["medi_wrench"]);
    carry(Slot::Utility, &["flamethrower"]);

    catalogue.loadouts.insert(Class::Mercenary, mercenary);
    catalogue
}
